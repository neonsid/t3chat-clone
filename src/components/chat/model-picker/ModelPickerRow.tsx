...
import { formatCost, formatTokenLimit } from "@/components/chat/model-picker/logic";
...
function describeModel(model: ModelCatalogEntry): string {
  ...
  const facts = [
    ...
    model.inputCostPerMillion !== null && `$${formatCost(model.inputCostPerMillion)} in`,
    model.outputCostPerMillion !== null && `$${formatCost(model.outputCostPerMillion)} out`,
          ...
          <Tooltip content={isFavorite ? "Remove from favorites" : "Add to favorites"}>
            <button
              ...
              className={cn(
                ...
                isFavorite ? "text-amber-400 opacity-100" : "opacity-60 focus-visible:opacity-100",
              )}
            >
              <StarIcon className={cn("size-3.5", isFavorite && "fill-amber-400 text-amber-400")} />
            </button>
          </Tooltip>
          {model.experimental ? (
