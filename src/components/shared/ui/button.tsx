...
/**
 * Hover surfaces use `accent` rather than `muted`. In dark mode `--muted` is
 * `rgb(255 255 255 / 4%)`, so the stock `dark:hover:bg-muted/50` resolved to 2%
 * white and was invisible on this app's near-black canvas. Those `dark:`
 * overrides are gone rather than retuned for two reasons: the extra
 * `:is(.dark *)` outranked any plain `hover:bg-*` a caller passed in, and
 * tailwind-merge cannot dedupe across differing modifiers, so callers had no way
 * to override the hover without `!important`.
 */
